...
- **Accommodation Availability**: Live bungalow tally (5 bungalows, each = 1 family room + 1 single room; a whole booking consumes both rooms of a unit; whole-bungalow availability uses exact capacity `5 - W - max(S, F)` since rooms can be repacked into shared units) via `/api/accommodation-availability`; remaining counts show in the form, sold-out options disable, and POST `/api/registration` rejects sold-out options with 409.
...
- **i18n**: next-intl (EN/PT, locale-prefixed public routes)
